  ...
  return (
    <div className="p-8 flex-1 bg-white">
      ...
      <div className="flex space-x-4 mb-6 text-sm font-medium">
        {["All", "Office", "Residential", "Covered", "EV Charging"].map(
          (cat) => (
        ...
        {listings.map((space, index) => (
          <div key={index} className="bg-purple-50 p-4 rounded-xl shadow-sm">
            ...
            <div className="text-sm text-gray-600">
              {space.spots} spots • ${space.price}/hr • {space.details}
            </div>
            <div className="flex gap-2 mt-4">
              <button className="bg-purple-600 text-white px-3 py-1 rounded-md text-sm">
                Book Now
              </button>
              <button className="border border-purple-600 text-purple-600 px-3 py-1 rounded-md text-sm">
                Details
              </button>
            </div>
          </div>
      ...
      <img
        src="https://upload.wikimedia.org/wikipedia/commons/9/95/Europe_Map_1851.jpg"
        alt="Map"
